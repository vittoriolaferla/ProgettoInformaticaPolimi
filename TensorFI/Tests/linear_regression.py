...
with tf.Session() as sess:
    ...
    fi = ti.TensorFI(sess, name = "linearReg", logLevel = 30, disableInjections = True)
    
    # Calculate accuracy (with no fault injections)
    print("Accuracy (no injections):", sess.run(accuracy, feed_dict={X: test_X, Y: test_Y}))

    # Calculate accuracy (with fault injections)
    fi.turnOnInjections()	
    print("Accuracy (with injections):", sess.run(accuracy, feed_dict={X: test_X, Y: test_Y}))
